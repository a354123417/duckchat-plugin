...
<head>
    ...
    <link rel="stylesheet" href="../../public/css/grab.css?v=4"/>
</head>
<style>
    .red_back {
        width: 325px;
...
<div class="wrapper">
    ...
    <div class="red-center open_btn" style="position: relative">
        <?php if ($isGrabbedOver) { ?>
            <div class="red-grab-over" style="position: absolute;bottom:50px;"
                 onclick="showDetails('<?php echo $packetId; ?>');">点击查看领取详情
            </div>
        <?php } else { ?>
            <button class="red-open-button" style="position: absolute;bottom:100px;"
                    onclick="grabRedPacket('<?php echo $packetId; ?>')">開
            </button>
        <?php } ?>
    </div>
